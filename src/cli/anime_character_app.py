...
# ===== 1. 训练相关配置 =====
# ======================
DATASET_DIR = 'E:\pj\TAC_dataset\processed' 			# 训练集
MODEL_SAVE_PATH = './models/character_resnet18.pth'
CLASSES_TXT_PATH = './models/classes.txt'      		# 保存类别名（每行一个）
NUM_EPOCHS = 10
BATCH_SIZE = 16
IMAGE_SIZE = 224
LEARNING_RATE = 1e-4
...
# ===== 2. 预测相关配置=====
# ======================
MODEL_LOAD_PATH = './models/character_resnet18.pth'  # 模型文件
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            with Image.open(file_path) as img:
                ...
                if width <= 0 or height <= 0:
                    logger.warning(f"图片尺寸异常（宽或高 <= 0） {file_path}: {width}x{height}")
                    return False
                ...
                try:
                    ...
                    logger.warning(f"图片色彩模式异常，无法转为 RGB {file_path}: {str(e)}")
                    return False
                ...
                exif_warning_detected = any(
                    "Corrupt EXIF data" in str(warn.message) or 
                    "Expecting to read 4 bytes but only got 0" in str(warn.message)
                    for warn in w if warn.category == UserWarning
                )
            ...
            Image.UnidentifiedImageError, ValueError, IndexError) as e:
...
def calculate_file_hash(file_path: str, hash_algorithm: str = 'md5', buffer_size: int = 65536) -> Optional[str]:
    """计算文件的哈希值"""
    try:
...
def validate_dataset_images(dataset_dir: str) -> Tuple[int, int, List[str], List[str]]:
    ...
    try:
        ...
        if invalid_image_paths:
            logger.error(f"共发现 {len(invalid_image_paths)} 个无效图像文件，来自以下 {len(invalid_classes)} 个类别: {invalid_classes}")
...
def verify_images_function():
    ...
    if not os.path.exists(DATASET_DIR):
        logger.error(f"数据集目录不存在: {DATASET_DIR}")
        logger.error("请先创建数据集目录，并为每个角色创建一个子文件夹，子文件夹内放入对应角色的图片。")
        return
    ...
    # 验证数据集中的图像文件
    valid_samples, total_samples, class_names, invalid_image_paths = validate_dataset_images(DATASET_DIR)
...
def train_model():
    ...
    if not os.path.exists(DATASET_DIR):
        logger.error(f"数据集目录不存在: {DATASET_DIR}")
        logger.error("请先创建数据集目录，在每个IP文件夹下，为每个角色创建一个子文件夹，子文件夹内放入对应角色的图片。")
        return
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            for inputs, labels in loop:
                try:
                    ...
                    loop.set_postfix(loss=f"{running_loss/(loop.n+1):.4f}", acc=f"{100.*current_acc:.2f}%")
                except Exception as e:
            ...
            logger.info(f"第 {epoch+1} 轮完成 | 损失: {epoch_loss:.4f} | 正确率: {epoch_acc:.2f}%")
            
            if epoch_acc > best_accuracy:
                best_accuracy = epoch_acc
                torch.save(model.state_dict(), MODEL_SAVE_PATH)
                logger.info(f"保存最佳模型 (准确率: {best_accuracy:.2f}%) 到: {MODEL_SAVE_PATH}")
...
def predict_character():
    ...
    # --- 3. 用户输入图片路径 ---
    while True:
        user_input = input("Hiahiahia… 请输入你要预测的图片路径（或输入 0 返回主菜单）: ").strip()
        if user_input == '0':
...
def main():
    ...
    if args.mode == 1:
        ...
        logger.info("\n=== 4. 启动节点服务 ===")
        print("7888")
        from node_server import start_node_service
        start_node_service()
        return
